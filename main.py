...
from deepseek_api import careHeart
import numpy as np
...
sys.path.append("F:/python/visualization/chat_win")
ONNX_MODEL_PATH = "F:/python/visualization/chat_win/model/model.onnx"  # ONNX模型路
def get_data():
    # return torch.rand(64, 500)
    return np.random.randn(1, 1,64,500).astype(np.float32)
net1=net(ONNX_MODEL_PATH,4)

# net1 = net('../model/nn_dict_4.pth', 4)
# print(net1.predict(get_data()))
url = "https://openrouter.ai/api/v1"
api_key = "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"
model = "deepseek/deepseek-r1:free"
care=careHeart(url, api_key, model)
# care=careHeart()
...
def return_message():
    ...
    if session.get('user_id') is None:  # 如果当前session未绑定用户
        ...
        if send_message.startswith("id:"):
            ...
            keywords=["抑郁","心理","健康","精神","depression","mental","health","psychological","yiyv","xinli","jiankang","jingshen"]
            ...
            for keyword in keywords:
                if keyword in send_message:
                    ...
                    with open("result.txt", "r", encoding="utf-8") as f:
                        loaded_data = f.read()
                    user_input = f"{loaded_data}\n\n{send_message}"
                    break
                
            answer = care.answer(user_input)



            # 返回固定内容
            response=keep_second_part(answer, delimiter="</think>")
            ...
            if not STREAM_FLAG:
                ...
                if save_message:
                    messages_history.append({'role': 'user', 'content': send_message})
                asyncio_run(save_all_user_dict())
                if not save_message:
                    messages_history = []
...
def keep_second_part(text, delimiter="aaaaaaaaaaaaaaa"):
    """
    分割文本并保留后半部分

    参数:
        text (str): 包含分割符的完整文本
        delimiter (str): 分割符，默认为"aaaaaaaaaaaaaaa"

    返回:
        str: 后半部分文本，如果找不到分割符则返回原始文本
    """
    parts = text.split(delimiter)
    if len(parts) > 1:
        return parts[1].strip()  # 返回后半部分并去除首尾空白
    return text  # 找不到分割符时返回原文本
...
if __name__ == '__main__' or __name__ == 'main':
    ...
    # webbrowser.open("http://172.16.46.77:5000")
